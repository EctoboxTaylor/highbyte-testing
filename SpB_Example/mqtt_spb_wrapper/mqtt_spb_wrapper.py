...
_log_handle.setFormatter(logging.Formatter(
...
class MqttSpbTopic:
    ...
    def parse_topic(self, topic_str):
        ...
        self.domain = "%s.%s.%s" % (
            self.namespace, self.group_name, self.eon_name)
...
class MqttSpbPayload:
    ...
    def parse_payload(self, payload_data):
        ...
        try:
            ...
            # Convert it to DICT for easy handeling
            ...
            try:
                ...
                logger.error(
                    "Could not parse MQTT CMD payload, message ignored ! (reason: %s)" % (str(e1)))
                return None
            ...
            logger.error(
                "Could not parse MQTT CMD payload, message ignored ! (reason: %s)" % (str(e)))
...
class MqttSpbEntity:
    ...
    def __init__(self, spb_group_name, spb_eon_name,
        ...
        if spb_eon_device_name is None:
            self._entity_domain = "spBv.10.%s.%s" % (
                self._spb_group_name, self._spb_eon_name)
        else:
            self._entity_domain = "spBv.10.%s.%s.%s" % (
                self._spb_group_name, self._spb_eon_name, self._spb_eon_device_name)
    ...
    def serialize_payload_birth(self):
        ...
        if not self.attributes.is_empty():
            for item in self.attributes.values:
                name = "ATTR/" + item.name
                addMetric(payload, name, None, self._spb_data_type(
                    item.value), item.value, item.timestamp)

        # Data
        if not self.data.is_empty():
            for item in self.data.values:
                name = item.name  # CHANGED 20240422 - PI Historian doesnt want the Data birth fields prefixed with "DATA/"
                # name = "DATA/" + item.name
                addMetric(payload, name, None, self._spb_data_type(
                    item.value), item.value, item.timestamp)
            ...
            for field in payload.get('metrics', []):
                ...
                if field['name'].startswith("ATTR/"):
                    ...
                    self.attributes.set_value(
                        # update field
                        field['name'], field['value'], field['timestamp'])
    ...
    def serialize_payload_data(self, send_all=False):
        ...
        for item in self.data.values:
            ...
            if send_all or item.is_updated:
                addMetric(payload, item.name, None, self._spb_data_type(
                    item.value), item.value, item.timestamp)
    ...
    def publish_birth(self):
        ...
        if not self.is_connected():  # If not connected
            logger.warning(
                "%s - Could not send publish_birth(), not connected to MQTT server" % self._entity_domain)
            return False
        ...
        if self._entity_is_scada:
            ...
            logger.info("%s - Published STATE BIRTH message " %
                        (self._entity_domain))
            return
        ...
        if self._spb_eon_device_name is None:  # EoN
            ...
            topic = "spBv1.0/" + self.spb_group_name + "/DBIRTH/" + \
                self._spb_eon_name + "/" + self._spb_eon_device_name
        self._loopback_topic = topic
    ...
    def publish_data(self, send_all=False):
        ...
        if send_all or self.data.is_updated():
            ...
            payload_bytes = self.serialize_payload_data(
                send_all)   # Get the data payload

            if self._spb_eon_device_name is None:  # EoN
                topic = "spBv1.0/" + self.spb_group_name + "/NDATA/" + self._spb_eon_name
            else:
                topic = "spBv1.0/" + self.spb_group_name + "/DDATA/" + \
                    self._spb_eon_name + "/" + self._spb_eon_device_name
            self._loopback_topic = topic
            self._mqtt.publish(topic, payload_bytes, 0, False)

            logger.info("%s - Published DATA message %s" %
                        (self._entity_domain, topic))
            return True

        logger.warning(
            "%s - Could not publish DATA message, may be data no new data values?" % (self._entity_domain))
        return False
    ...
    def connect(self,
        ...
        if tls_cert_path and tls_cert_path and tls_key_path:
            ...
            import ssl
            self._mqtt.tls_set(ca_certs=tls_cert_path, certfile=tls_cert_path,
                               keyfile=tls_key_path, cert_reqs=ssl.CERT_NONE)
        ...
        if self._entity_is_scada:  # If it is a type entity SCADA, change the DEATH certificate
            topic = "spBv1.0/" + self.spb_group_name + "/STATE/" + self._spb_eon_name
            self._mqtt.will_set(topic, "OFFLINE".encode(
                "utf-8"), 0, True)  # Set message
        else:  # Normal node
            ...
            if self._spb_eon_device_name is None:  # EoN
                ...
                topic = "spBv1.0/" + self.spb_group_name + "/DDEATH/" + \
                    self._spb_eon_name + "/" + self._spb_eon_device_name
            self._mqtt.will_set(topic, payload_bytes, 0, True)  # Set message

        # MQTT Connect
        logger.info("%s - Trying to connect MQTT server %s:%d" %
                    (self._entity_domain, host, port))
        try:
            self._mqtt.connect(host, port)
        except Exception as e:
            logger.warning("%s - Could not connect to MQTT server (%s)" %
                           (self._entity_domain, str(e)))
            return False
    ...
    def disconnect(self):
        ...
        logger.info("%s - Disconnecting from MQTT server" %
                    (self._entity_domain))
    ...
    def _mqtt_on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            if self._spb_eon_device_name is None:  # EoN
                ...
                topic = "spBv1.0/" + self.spb_group_name + "/DCMD/" + \
                    self._spb_eon_name + "/" + self._spb_eon_device_name
            self._mqtt.subscribe(topic)
            logger.info("%s - Subscribed to MQTT topic: %s" %
                        (self._entity_domain, topic))
            ...
            logger.error(" %s - Could not connect to MQTT server !" %
                         self._entity_domain)
    ...
    def _mqtt_on_message(self, client, userdata, msg):
        ...
        msg_ts_rx = int(round(datetime.datetime.now().utcnow().time() * 1000))

        logger.info("%s - Message received  %s" %
                    (self._entity_domain, msg.topic))
        ...
        if "CMD" in topic.message_type:
            ...
            if not payload['metrics']:
                logger.error(
                    "%s - CMD message IGNORED, incorrect or no CMDs provided" % (self._entity_domain))
            else:
                # Execute the callback function if it is not None
    ...
    class _ValuesGroup:
        ...
        def set_value(self, name, value, timestamp=None):
            ...
            self.values.append(
                MqttSpbEntityDevice._ValueItem(name, value, timestamp))
...
class MqttSpbEntityDevice(MqttSpbEntity):
    ...
    def __init__(self, spb_group_name, spb_eon_name, spb_eon_device_name,
        ...
        super().__init__(spb_group_name, spb_eon_name,
                         spb_eon_device_name, debug_info, filter_cmd_msg)
...
class MqttSpbEntityEdgeNode(MqttSpbEntity):
    ...
    def publish_command_device(self, spb_eon_device_name, commands):
        ...
        for k in commands:
            addMetric(payload, k, None, self._spb_data_type(
                commands[k]), commands[k])

        # Send payload if there is new data
        topic = "spBv1.0/" + self.spb_group_name + "/DCMD/" + \
            self._spb_eon_name + "/" + spb_eon_device_name
        ...
        if payload.metrics:
            ...
            logger.info("%s - Published COMMAND message to %s" %
                        (self._entity_domain, topic))

            return True

        logger.warning("%s - Could not publish COMMAND message to %s" %
                       (self._entity_domain, topic))
        return False
...
class MqttSpbEntityScada(MqttSpbEntity):
    ...
    def publish_command_device(self, spb_eon_name, spb_eon_device_name, commands):
        ...
        # Send payload if there is new data
        topic = "spBv1.0/" + self.spb_group_name + "/DCMD/" + \
            spb_eon_name + "/" + spb_eon_device_name
